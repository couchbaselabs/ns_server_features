...
apply_net_config(NodeKVList) ->
    ProtosB =
        case proplists:get_value(<<"dist_protocols">>, NodeKVList) of
            Ps when Ps =:= <<"undefined">>; Ps =:= undefined ->
                [<<"inet_tcp_dist">>, <<"inet6_tcp_dist">>];
            Ps -> Ps
        end,
    Protos = [binary_to_atom(P, latin1) || P <- ProtosB],
    AFamilyBin = proplists:get_value(<<"address_family">>, NodeKVList,
                                     <<"inet">>),
    AFamily = binary_to_atom(AFamilyBin, latin1),
    CEncription = proplists:get_value(<<"cluster_encryption">>, NodeKVList, false),
    ?log_info("Appling net config. AFamily: ~p, CEncryption: ~p, "
...
do_add_node_with_connectivity(Scheme, RemoteAddr, RestPort, Auth, GroupUUID,
    ...
    ConnectOpts =
        case Scheme of
            https -> ns_server_cert:tls_server_validation_options();
            http -> []
        end,
